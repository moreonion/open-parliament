...
WANTED_FIELDS = ['first_name', 'last_name', 'salutation', 'title', 'political_affiliation',
                 'state', 'occupation', 'place_of_birth', 'date_of_birth']
...
def convert_to_csv(jsonfile, output):
    ...
    for mp_data in sorted_input:
        ...
        if len(emails) > 0:
            ...
            mp['email'] = ''
            logger.warning('Contact without email: %s (%s %s)', mp_data['id'],
                           mp_data['first_name'], mp_data['last_name'])
        mp.update({k: mp_data[k] for k in WANTED_FIELDS})
        committees = sorted([c['name'] for f in mp_data['committees'].values() for c in f.values()])
        ...
        if party and party.group('party'):
            ...
            mp['party'] = ''
            logger.warning('Contact without party: %s (%s %s)', mp_data['id'],
                           mp_data['first_name'], mp_data['last_name'])
        contacts.append(mp)
        ...
        mp['display_name'] = '{} {}{}{}'.format(mp_data['last_name'], mp_data['first_name'], display_title, display_party)

    output_mapping = ['identifier', 'first_name', 'last_name', 'email', 'salutation',
                      'display_name', 'title', 'party', 'political_affiliation', 'state', 'occupation',
                      'place_of_birth', 'date_of_birth', 'mandates', 'committees']
